                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    <form action="{{ route('audits.update', $audit->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="date" id="date" name="date" value="{{ $audit->date }}" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                        </div>
                        <div class="mb-4">
                            ...
                            <select id="mode" name="mode" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                @foreach(\App\Enums\AuditMode::cases() as $mode)
                                    <option value="{{ $mode->value }}" {{ $audit->mode == $mode->value ? 'selected' : '' }}>{{ $mode->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select id="site_id" name="site_id" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                @foreach($sites as $site)
                                    <option value="{{ $site->id }}" {{ $audit->site_id == $site->id ? 'selected' : '' }}>{{ $site->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select id="contact" name="contact" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" required>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ $audit->contact == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        <div class="mb-4">
                            ...
                            <select id="is_virtual" name="is_virtual" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                                <option value="0" {{ $audit->is_virtual == 0 ? 'selected' : '' }}>No</option>
                                <option value="1" {{ $audit->is_virtual == 1 ? 'selected' : '' }}>Yes</option>
                            </select>
                        </div>
                        <div class="mb-4">
                            <label for="comment" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Comment</label>
                            <textarea id="comment" name="comment" rows="3" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">{{ $audit->comment }}</textarea>
                        </div>
                        <div class="mb-4">
                            <label for="follow_up_date" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Follow Up Date</label>
                            <input type="date" id="follow_up_date" name="follow_up_date" value="{{ $audit->follow_up_date }}" class="mt-1 text-black block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm">
                        </div>
                        <button type="submit" class="inline-flex items-center px-4 py-2 border text-sm font-medium rounded-md shadow-sm text-white bg-blue-500 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">Update Safety Walk</button>
                    </form>
                </div>
            </div>
        </div>
